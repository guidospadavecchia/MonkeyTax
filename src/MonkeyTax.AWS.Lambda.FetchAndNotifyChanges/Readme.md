...
WORKDIR /src
COPY ["MonkeyTax.AWS.Lambda.FetchAndNotifyChanges.csproj", "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges/"]
RUN dotnet restore "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges/MonkeyTax.AWS.Lambda.FetchAndNotifyChanges.csproj"

WORKDIR "/src/MonkeyTax.AWS.Lambda.FetchAndNotifyChanges"
COPY . .
RUN dotnet build "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges.csproj" \
...
Execute unit tests
```
cd "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges/test/MonkeyTax.AWS.Lambda.FetchAndNotifyChanges.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "MonkeyTax.AWS.Lambda.FetchAndNotifyChanges/src/MonkeyTax.AWS.Lambda.FetchAndNotifyChanges"
